...
PointCloud2BoxFilter::PointCloud2BoxFilter()
    : rclcpp::Node("pointcloud2_box_filter"), tf_buffer_(this->get_clock()),
      tf_listener_(tf_buffer_) {
  param_listener_ =
      std::make_shared<pointcloud2_box_filter_params::ParamListener>(
  ...
  sub_ = this->create_subscription<sensor_msgs::msg::PointCloud2>(
      ...
      std::bind(&PointCloud2BoxFilter::pointcloudCallback, this,
                std::placeholders::_1));
  bbox_pub_ = this->create_publisher<vision_msgs::msg::BoundingBox3D>(
      "~/filter_bounding_box", 10);
}

void PointCloud2BoxFilter::pointcloudCallback(
  ...
  bbox_msg.center.orientation.w = 1.0; // No rotation
...
//   bbox_msg.header.frame_id = params_.target_frame;
//   bbox_msg.header.stamp = this->get_clock()->now();
